...
function __autorestic_perform_completion
    __autorestic_debug "Starting __autorestic_perform_completion"
...
# - Obtain the completions and store them in the global __autorestic_comp_results
# - Return false if file completion should be performed
function __autorestic_prepare_completions
    __autorestic_debug ""
    __autorestic_debug "========= starting completion logic =========="

    # Start fresh
    set --erase __autorestic_comp_results

    set -l results (__autorestic_perform_completion)
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __autorestic_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__autorestic_comp_results)
        set --global __autorestic_comp_results $completions
        __autorestic_debug "Filtered completions are: $__autorestic_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__autorestic_comp_results)
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__autorestic_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                set --global __autorestic_comp_results $split[1] $split[1].
                __autorestic_debug "Completions are now: $__autorestic_comp_results"
            end
        end
...
# The call to __autorestic_prepare_completions will setup __autorestic_comp_results
